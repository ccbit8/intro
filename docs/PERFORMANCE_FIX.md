...
### 🎨 视觉稳定性与渲染性能优化报告 (Visual Stability & Rendering Fixes)

## 🎯 目标 (Objectives)
1.  **消除布局偏移 (CLS)**：解决图表加载导致的页面抖动。
2.  **修复视觉闪烁**：解决图表加载瞬间下方卡片列表 "刷新/重载" 的视觉 Bug。
3.  **优化资源加载**：减小首屏 Bundle 体积，提升 LCP (Least Contentful Paint)。

---

## 🛠️ 关键修复一：Webpack 分包优化 (Chunk Splitting)

### 问题
`Recharts` (图表库) 和 `Lucide-react` (图标库) 被打包进了主 `main-app` bundle，导致首屏 JS 体积过大，阻塞水合 (Hydration)。

### 解决方案
在 `next.config.js` 中配置自定义 Webpack 规则，强制拆分大依赖。
...
## 🛠️ 关键修复二：消除布局偏移 (Fixing CLS)

### 问题
用户反馈 "图表加载完，下面的 Card 会全部刷新一下"。
根本原因是 **骨架屏 (Skeleton)** 与 **真实图表 (Real Component)** 的 DOM 结构和尺寸不一致。
*   **Skeleton**: 仅包含内部 `aspect-square` 的 DIV。
*   **Real Chart**: 包含了外层 `p-4` padding wrapper 和底部的 Footer 区域。
*   **Recharts 行为**: 默认根据 Label 长度自动计算半径，导致每次渲染半径可能跳变。

### 解决方案

#### 1. 对齐骨架屏结构 (`radar-skeleton.tsx`)
重写 SVG 和容器结构，使其像素级 (Pixel-perfect) 匹配真实组件。
*   添加外层 `p-4` Padding。
*   添加底部 Footer 占位符 (`mt-2`, `h-3`)。
*   重新计算 SVG 六边形路径，严格匹配图表半径。

#### 2. 锁定图表几何尺寸 (`radar-chart-04/index.tsx`)
强制固定 Recharts 的半径参数，防止自动计算导致的跳动。

```tsx
<RadarChart
  outerRadius="70%" // ✅ 锁定半径，不依赖自动布局
  animationDuration={0} // ✅ 禁用生长动画，消除"刷新感"
  // ...
/>
```
...
## 🛠️ 关键修复三：关键资源锁定 (Critical Resource Locking)

### 问题
即使解决了布局偏移，首屏下方的卡片图片 (Next.js Image) 仍然会出现瞬间闪烁。
这是由于浏览器在发生 **Layer Invalidation (合成层失效)** 和 **Reflow (重排)** 时，为了节省内存，临时卸载/重解码了 `loading="lazy"` 的图片资源。

### 解决方案

#### 1. 视口资源优先加载 (`page.tsx`)
为首屏可见区域的前 4 张卡片开启 `priority` 属性。这告诉浏览器这些图片在布局抖动时也是"关键资源"，必须保留在渲染管线中。

> 📖 **扩展阅读**: 为什么只加了前 4 张，整个页面都不闪了？[深度解析：浏览器渲染机制与图片重载谜题](./RENDERING_DEEP_DIVE.md)
...
{projects.map((project, index) => (
  <Card 
    key={project.name} 
    project={project} 
    priority={index < 4} // ✅ 前4张图 (首屏) 强制 Eager Loading，防止重排时闪烁
  />
))}
```

#### 2. 防止无谓重渲染 (`card.tsx`)
使用 `React.memo` 包装卡片组件，确保父组件 (Page) 在 Hydration 过程中更新时，数据未变的子组件通过缓存跳过 Diff 过程。
...
## 📊 优化结果
*   **Visual Stability**: 页面加载过程平滑，图表从骨架屏切换到实图无肉眼可见的位移。
*   **LCP Score**: 首屏关键图片提前加载，提升了 Core Web Vitals 分数。
*   **User Experience**: 彻底消除了"图片全部刷新/闪一下"的视觉 Bug。
...
> 💡 **深度阅读**：
> *   [📦 Webpack 打包策略深度解析：为什么我的 Icon 包里有图表？](./WEBPACK_SPLIT_CHUNKS_ISSUE.md)
> *   [📏 Webpack Chunk 拆分黄金法则：代码到底要拆多细？](./WEBPACK_CHUNK_SPLITTING_GUIDE.md)
